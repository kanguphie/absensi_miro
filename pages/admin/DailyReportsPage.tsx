...
import { FiClipboard, FiPrinter, FiUserCheck, FiClock, FiAlertCircle, FiHelpCircle } from 'react-icons/fi';
...
const DailyReportsPage: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    const printHtml = `<!DOCTYPE html><html lang="id"><head><title>Laporan Harian - ${date}</title><style>body { font-family: 'Segoe UI', 'Helvetica Neue', Arial, sans-serif; margin: 0; font-size: 10pt; background-color: #fff; color: #333; } .page-container { padding: 1.5cm; } .header { display: flex; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #333; padding-bottom: 15px; } .header img { height: 60px; width: 60px; object-fit: contain; margin-right: 20px; } .header-text { flex-grow: 1; } .header-text h1 { margin: 0; font-size: 18pt; font-weight: 600; } .header-text h2 { margin: 0; font-size: 12pt; font-weight: 400; color: #555; } .report-info { text-align: right; font-size: 10pt; } .summary-grid { display: flex; justify-content: space-around; text-align: center; margin: 25px 0; padding: 15px; background-color: #f9f9f9; border-radius: 8px; border: 1px solid #eee; } .summary-item .count { font-size: 20pt; font-weight: bold; } .summary-item .label { font-size: 9pt; color: #666; } .content-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; } .status-section { border: 1px solid #ddd; border-radius: 8px; page-break-inside: avoid; overflow: hidden; } .status-section h3 { margin: 0; padding: 10px 15px; font-size: 11pt; border-bottom: 1px solid #ddd; } .status-late h3 { background-color: #fef3c7; border-left: 5px solid #f59e0b; } .status-absent h3 { background-color: #fee2e2; border-left: 5px solid #ef4444; } .status-sick h3 { background-color: #dbeafe; border-left: 5px solid #3b82f6; } .status-permit h3 { background-color: #e0e7ff; border-left: 5px solid #6366f1; } table.student-list { width: 100%; border-collapse: collapse; } .student-list th { text-align: left; padding: 8px 15px; background-color: #f7f7f7; font-weight: 600; } .student-list td { padding: 8px 15px; border-top: 1px solid #eee; } .student-list .nis { font-family: 'Courier New', Courier, monospace; color: #555; } .no-data { padding: 20px; text-align: center; color: #888; } .footer { margin-top: 30px; padding-top: 15px; border-top: 1px solid #ccc; font-size: 8pt; color: #777; text-align: center; } @media print { @page { size: A4 portrait; margin: 0; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } }</style></head><body><div class="page-container"><div class="header">${settings?.schoolLogoUrl ? `<img src="${settings.schoolLogoUrl}" alt="Logo">` : ''}<div class="header-text"><h1>Laporan Kehadiran Harian</h1><h2>${settings?.schoolName || 'Sistem Absensi RFID'}</h2></div><div class="report-info"><strong>Tanggal:</strong><br>${date}<br><strong>Kelas:</strong> ${className}</div></div><div class="summary-grid"><div class="summary-item"><div class="count">${present.length + late.length}</div><div class="label">Total Hadir</div></div><div class="summary-item"><div class="count">${present.length}</div><div class="label">Tepat Waktu</div></div><div class="summary-item"><div class="count">${late.length}</div><div class="label">Terlambat</div></div><div class="summary-item"><div class="count">${absent.length}</div><div class="label">Belum Hadir</div></div><div class="summary-item"><div class="count">${sick.length + permit.length}</div><div class="label">Izin/Sakit</div></div></div><div class="content-grid">${Object.entries(studentGroupsToPrint).map(([status, studentList]) => { const detail = groupDetails[status as keyof typeof groupDetails]; return `<div class="status-section ${detail.class}"><h3>${detail.title} (${studentList.length})</h3>${studentList.length > 0 ? `<table class="student-list"><thead><tr><th>Nama Siswa</th><th>NIS</th></tr></thead><tbody>${studentList.map(s => `<tr><td>${s.name}</td><td class="nis">${s.nis}</td></tr>`).join('')}</tbody></table>` : `<p class="no-data">Tidak ada data</p>`}</div>` }).join('')}</div><div class="footer">Dicetak pada: ${new Date().toLocaleString('id-ID')}</div></div></body></html>`;
